...
<div class="container-fluid">
    ...
    <h1 class="mt-4 mabo64">Admin</h1>
    <div class="row mt-4">
        @if(Auth::user()->role == 99)
        <div class="col-12 col-md-4">
            <form class="px-4 py-3" action="/addAdmin" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if(Auth::user()->role == 99)
                            @if($admin->status == 1)
                            <a href="/adminPending/{{$admin->id}}" class="material-icons" style="font-size: 14px">pause</a>
                            @else
                            <a href="/adminPlay/{{$admin->id}}" class="material-icons" style="font-size: 14px">play_arrow</a>
                            @endif
                            <a href="/adminDelete/{{$admin->id}}" class="material-icons" style="font-size: 14px">delete</a>
                            </span>
                            @else
                            @if($admin->status == 1)
                            <a href="/adminPending/{{$admin->id}}" class="material-icons" style="font-size: 14px">pause</a>
